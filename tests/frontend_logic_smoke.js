...
async function main() {
  ...
  const fixture = {
    ...
    clients: [
      ...
      { id: 'c_brown_missing', phone: '0733333334', note: 'brown missing', createdAt: iso, history: [], assignments: [assignment('a3b', 'e1', 'brown'), assignment('a4b', 'e2', 'missing')] },
  ...
  const context = {
    ...
    fetch: async (_url, options) => {
      ...
      if (context.failNextSave) {
        context.failNextSave = false;
        return { status: 500, ok: false, json: async () => ({}), text: async () => 'save failed' };
      }
      rev += 1;
  ...
  script += `
    (async () => {
      ...
      assert(db.clients.length === 8, 'Clienții cu același telefon nu au fost uniți.');
      ...
      assert(bulkArchiveCandidates('brown').length === 3, 'Număr greșit de candidați MARO.');
      assert(bulkArchiveCandidates('red').length === 4, 'Număr greșit de candidați ROȘU.');
      ...
      assert(db.clients.length === 8, 'Starea locală nu a revenit după eroarea de salvare.');
      assert(db.archivedClients.length === 0, 'Arhivarea eșuată a rămas vizibilă local.');

      await bulkArchiveStatus('brown');
      await bulkArchiveStatus('red');

      assert(db.archivedClients.length === 7, 'Nu au fost arhivate toate numerele eligibile.');
      assert(db.clients.length === 1, 'Au rămas numere eligibile nearhivate.');
      assert(db.clients.some(c => c.id === 'c_conflict'), 'Un conflict ACTIV + ROȘU a fost arhivat greșit.');
      assert(db.archivedClients.some(c => c.phone === '0755555555'), 'Un telefon MARO + ROȘU nu a fost arhivat de acțiunea ROȘU.');
      assert(posts.length === 3, 'Arhivarea în masă nu a făcut numărul corect de salvări, inclusiv testul de eroare.');
    })()
  `;
